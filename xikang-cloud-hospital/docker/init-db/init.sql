...
COMMENT ON COLUMN regist_level.delmark IS '软删除标记: 0-有效, 1-已删除（与 mapper 中 WHERE delmark = 0 约定一致）';
...
CREATE TABLE register (
    ...
    scheduling_id           BIGINT          DEFAULT NULL,
    ...
    clinical_archived_at    TIMESTAMP       DEFAULT NULL,
    clinical_archived_by    INTEGER         DEFAULT NULL,
    ...
    CONSTRAINT chk_register_visit_state CHECK (visit_state IN (1, 2, 3, 4, 5, 6, 7)),

    CONSTRAINT chk_register_regist_money CHECK (regist_money >= 0)
);

CREATE INDEX idx_register_patient_id ON register(patient_id);
CREATE INDEX idx_register_scheduling_id ON register(scheduling_id);
...
CREATE INDEX idx_register_clinical_archived ON register(patient_id, clinical_archived_at DESC NULLS LAST);

-- ============================================================
-- 表: patient_clinical_profile (患者长期临床档案)
-- ============================================================
...
CREATE INDEX idx_expense_record_register_id ON expense_record(register_id);
...
-- 说明: 真实钱包账本：每次充值/扣款/退款都写一条流水，与 patient.account_balance 原子一致
-- ============================================================
CREATE TABLE patient_balance_transaction (
    id                BIGSERIAL       PRIMARY KEY,
...
CREATE INDEX idx_patient_balance_transaction_patient_time
    ON patient_balance_transaction(patient_id, transaction_time DESC);

CREATE INDEX idx_patient_balance_transaction_business
    ON patient_balance_transaction(patient_id, transaction_type, business_type, business_id);

CREATE UNIQUE INDEX uk_patient_balance_transaction_business_unique
    ON patient_balance_transaction(patient_id, transaction_type, business_type, business_id)
...
CREATE TABLE ai_medical_record_log (
    ...
    CONSTRAINT chk_ai_mrlog_source CHECK (source_type IN ('consultation', 'dictation', 'exam', 'preliminary_diagnosis', 'w2_recommend', 'w4_diagnose')),
...
-- ============================================================
-- 表: follow_up_health_metric (随访疗效评估模拟健康指标)
-- ============================================================
...
CREATE INDEX idx_followup_metric_register_date ON follow_up_health_metric(register_id, record_date);

COMMENT ON TABLE follow_up_health_metric IS '随访疗效评估模拟健康指标表（EAV）';

-- ============================================================
-- 表: follow_up_interview_schedule (随访每周患者访谈日程)
-- ============================================================
...
CREATE INDEX idx_followup_interview_week ON follow_up_interview_schedule(week_start_date);
...
INSERT INTO scheduling (id, rule_name, week_rule, delmark) VALUES
    (1, '周一至周五上午', '1上2上3上4上5上', 0),
    (2, '周一至周五下午', '1下2下3下4下5下', 0),
    (3, '周六上午', '6上', 0)
...
-- 说明：mapper 约定 delmark = 0 = 有效，所以种子数据显式指定 delmark = 0
-- 科室1: 内科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (1, 1, 1, '内科张医生', 'dev-password', 0),
    (1, 1, 2, '内科李医生', 'dev-password', 0),
    (1, 1, 3, '内科王医生', 'dev-password', 0),
    (1, 2, 1, '内科赵专家', 'dev-password', 0),
    (1, 3, 1, '内科刘主任', 'dev-password', 0);
-- 科室2: 呼吸内科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (2, 1, 1, '呼吸内科周医生', 'dev-password', 0),
    (2, 1, 2, '呼吸内科吴医生', 'dev-password', 0),
    (2, 1, 3, '呼吸内科郑医生', 'dev-password', 0),
    (2, 2, 1, '呼吸内科冯专家', 'dev-password', 0),
    (2, 3, 1, '呼吸内科陈主任', 'dev-password', 0);
-- 科室3: 心血管内科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (3, 1, 1, '心血管内科孙医生', 'dev-password', 0),
    (3, 1, 2, '心血管内科李医生', 'dev-password', 0),
    (3, 1, 3, '心血管内科林医生', 'dev-password', 0),
    (3, 2, 1, '心血管内科何专家', 'dev-password', 0),
    (3, 3, 1, '心血管内科高主任', 'dev-password', 0);
-- 科室4: 消化内科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (4, 1, 1, '消化内科马医生', 'dev-password', 0),
    (4, 1, 2, '消化内科朱医生', 'dev-password', 0),
    (4, 1, 3, '消化内科秦医生', 'dev-password', 0),
    (4, 2, 1, '消化内科尤专家', 'dev-password', 0),
    (4, 3, 1, '消化内科许主任', 'dev-password', 0);
-- 科室5: 神经内科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (5, 1, 1, '神经内科施医生', 'dev-password', 0),
    (5, 1, 2, '神经内科张医生', 'dev-password', 0),
    (5, 1, 3, '神经内科蒋医生', 'dev-password', 0),
    (5, 2, 1, '神经内科韩专家', 'dev-password', 0),
    (5, 3, 1, '神经内科沈主任', 'dev-password', 0);
-- 科室6: 肾内科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (6, 1, 1, '肾内科唐医生', 'dev-password', 0),
    (6, 1, 2, '肾内科冯医生', 'dev-password', 0),
    (6, 1, 3, '肾内科董医生', 'dev-password', 0),
    (6, 2, 1, '肾内科潘专家', 'dev-password', 0),
    (6, 3, 1, '肾内科姜主任', 'dev-password', 0);
-- 科室7: 内分泌科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (7, 1, 1, '内分泌科苏医生', 'dev-password', 0),
    (7, 1, 2, '内分泌科魏医生', 'dev-password', 0),
    (7, 1, 3, '内分泌科卢医生', 'dev-password', 0),
    (7, 2, 1, '内分泌科崔专家', 'dev-password', 0),
    (7, 3, 1, '内分泌科蔡主任', 'dev-password', 0);
-- 科室8: 外科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (8, 1, 1, '外科丁医生', 'dev-password', 0),
    (8, 1, 2, '外科沈医生', 'dev-password', 0),
    (8, 1, 3, '外科徐医生', 'dev-password', 0),
    (8, 2, 1, '外科蒋专家', 'dev-password', 0),
    (8, 3, 1, '外科沈主任', 'dev-password', 0);
-- 科室9: 骨科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (9, 1, 1, '骨科卢医生', 'dev-password', 0),
    (9, 1, 2, '骨科马医生', 'dev-password', 0),
    (9, 1, 3, '骨科龚医生', 'dev-password', 0),
    (9, 2, 1, '骨科秦专家', 'dev-password', 0),
    (9, 3, 1, '骨科谢主任', 'dev-password', 0);
-- 科室10: 妇产科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (10, 1, 1, '妇产科苏医生', 'dev-password', 0),
    (10, 1, 2, '妇产科韦医生', 'dev-password', 0),
    (10, 1, 3, '妇产科严医生', 'dev-password', 0),
    (10, 2, 1, '妇产科卫专家', 'dev-password', 0),
    (10, 3, 1, '妇产科武主任', 'dev-password', 0);
-- 科室11: 儿科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (11, 1, 1, '儿科陶医生', 'dev-password', 0),
    (11, 1, 2, '儿科俞医生', 'dev-password', 0),
    (11, 1, 3, '儿科任医生', 'dev-password', 0),
    (11, 2, 1, '儿科袁专家', 'dev-password', 0),
    (11, 3, 1, '儿科柳主任', 'dev-password', 0);
-- 科室12: 新生儿科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (12, 1, 1, '新生儿科毕医生', 'dev-password', 0),
    (12, 1, 2, '新生儿科郝医生', 'dev-password', 0),
    (12, 1, 3, '新生儿科邬医生', 'dev-password', 0),
    (12, 2, 1, '新生儿科安专家', 'dev-password', 0),
    (12, 3, 1, '新生儿科常主任', 'dev-password', 0);
-- 科室13: 眼科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (13, 1, 1, '眼科乐医生', 'dev-password', 0),
    (13, 1, 2, '眼科于医生', 'dev-password', 0),
    (13, 1, 3, '眼科傅医生', 'dev-password', 0),
    (13, 2, 1, '眼科康专家', 'dev-password', 0),
    (13, 3, 1, '眼科陆主任', 'dev-password', 0);
-- 科室14: 耳鼻咽喉科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (14, 1, 1, '耳鼻咽喉科柴医生', 'dev-password', 0),
    (14, 1, 2, '耳鼻咽喉科胡医生', 'dev-password', 0),
    (14, 1, 3, '耳鼻咽喉科戴医生', 'dev-password', 0),
    (14, 2, 1, '耳鼻咽喉科蔡专家', 'dev-password', 0),
    (14, 3, 1, '耳鼻咽喉科谭主任', 'dev-password', 0);
-- 科室15: 口腔科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (15, 1, 1, '口腔科舒医生', 'dev-password', 0),
    (15, 1, 2, '口腔科屈医生', 'dev-password', 0),
    (15, 1, 3, '口腔科项医生', 'dev-password', 0),
    (15, 2, 1, '口腔科纪专家', 'dev-password', 0),
    (15, 3, 1, '口腔科梁主任', 'dev-password', 0);
-- 科室16: 皮肤科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (16, 1, 1, '皮肤科杜医生', 'dev-password', 0),
    (16, 1, 2, '皮肤科阮医生', 'dev-password', 0),
    (16, 1, 3, '皮肤科贝医生', 'dev-password', 0),
    (16, 2, 1, '皮肤科明专家', 'dev-password', 0),
    (16, 3, 1, '皮肤科程主任', 'dev-password', 0);
-- 科室17: 中医科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (17, 1, 1, '中医科卫医生', 'dev-password', 0),
    (17, 1, 2, '中医科申医生', 'dev-password', 0),
    (17, 1, 3, '中医科连医生', 'dev-password', 0),
    (17, 2, 1, '中医科习专家', 'dev-password', 0),
    (17, 3, 1, '中医科程主任', 'dev-password', 0);
-- 科室18: 肿瘤科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (18, 1, 1, '肿瘤科向医生', 'dev-password', 0),
    (18, 1, 2, '肿瘤科丁医生', 'dev-password', 0),
    (18, 1, 3, '肿瘤科茅医生', 'dev-password', 0),
    (18, 2, 1, '肿瘤科左专家', 'dev-password', 0),
    (18, 3, 1, '肿瘤科甘主任', 'dev-password', 0);
-- 科室19: 急诊科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (19, 1, 1, '急诊科龙医生', 'dev-password', 0),
    (19, 1, 2, '急诊科万医生', 'dev-password', 0),
    (19, 1, 3, '急诊科柯医生', 'dev-password', 0),
    (19, 2, 1, '急诊科柯专家', 'dev-password', 0),
    (19, 3, 1, '急诊科支主任', 'dev-password', 0);
-- 科室20: 康复医学科
INSERT INTO employee (deptment_id, regist_level_id, scheduling_id, realname, password, delmark) VALUES
    (20, 1, 1, '康复科管医生', 'dev-password', 0),
    (20, 1, 2, '康复科蔡医生', 'dev-password', 0),
    (20, 1, 3, '康复科蒙医生', 'dev-password', 0),
    (20, 2, 1, '康复科应专家', 'dev-password', 0),
    (20, 3, 1, '康复科丁主任', 'dev-password', 0);

-- 医技科室人员（不挂号，执行检查/检验）
INSERT INTO employee (deptment_id, realname, password, delmark) VALUES
    (35, '放射科技师王一', 'dev-password', 0),
    (35, '放射科技师李二', 'dev-password', 0),
    (35, '放射科医生张三', 'dev-password', 0),
    (36, '超声科技师赵四', 'dev-password', 0),
    (36, '超声科医生钱五', 'dev-password', 0),
    (37, '检验科技师孙六', 'dev-password', 0),
    (37, '检验科医生周七', 'dev-password', 0),
    (38, '输血科技师吴八', 'dev-password', 0),
    (39, '病理科医生郑九', 'dev-password', 0),
    (40, '处置室护士长冯十', 'dev-password', 0),
    (40, '处置室护士李一', 'dev-password', 0),
    (41, '内镜中心技师周二', 'dev-password', 0),
    (41, '内镜中心医生陈二', 'dev-password', 0),
    (42, '手术室护士长周三', 'dev-password', 0),
    (42, '手术室麻醉师李三', 'dev-password', 0),
    (44, '供应中心护士长王四', 'dev-password', 0),
    (45, '药房药师张五', 'dev-password', 0),
    (45, '药房药师赵六', 'dev-password', 0);
...
-- 患者档案数据
INSERT INTO patient (id, real_name, id_card, gender, birthdate, phone, relation, is_primary, allergy_history, delmark) VALUES
    (1, '患者小明', '210102199001011234', '男', '1990-01-01', '13800138000', '本人', 1, '青霉素过敏', 0),
    (2, '张小华', '210102196503151234', '女', '1965-03-15', '13900139000', '母亲', 0, '无', 0),
    (3, '李建国', '210102196203201234', '男', '1962-03-20', '13900139001', '父亲', 0, '磺胺类过敏', 0)
...
-- 诊疗医生账号：绑定 doctor1 + 批量生成 doc_{employee_id}
UPDATE users SET employee_id = 1 WHERE username = 'doctor1';
...
-- 随访疗效评估演示数据（见 follow_up_outcome_seed.sql，内容内联如下）
