...
COMMENT ON TABLE critical_value_alert IS '危急值闭环工单';
COMMENT ON COLUMN critical_value_alert.source_type IS 'inspection/check/disposal';
COMMENT ON COLUMN critical_value_alert.status IS 'PENDING/ACKNOWLEDGED/HANDLED/ESCALATED/CLOSED';
...
COMMENT ON TABLE critical_value_rule IS '危急值识别规则（检验参考阈值）';
