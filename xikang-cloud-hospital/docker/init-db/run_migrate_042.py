#!/usr/bin/env python3
"""Apply migrate_042_follow_up_exam_metric_catalog.sql using xikang-cloud-hospital/.env DB profile."""
...
ENV_PATH = ROOT / ".env"
MIGRATION = ROOT / "docker" / "init-db" / "migrate_042_follow_up_exam_metric_catalog.sql"
...
def main() -> None:
    ...
    cur.execute(
        ...
        WHERE table_schema = 'public'
          AND table_name = 'follow_up_exam_metric_catalog'
        """
    )
    table_exists = cur.fetchone()[0]
    print(f"Verification: follow_up_exam_metric_catalog exists => {table_exists == 1}")

    cur.execute("SELECT COUNT(*) FROM follow_up_exam_metric_catalog")
    row_count = cur.fetchone()[0]
    print(f"Verification: catalog row count => {row_count}")
