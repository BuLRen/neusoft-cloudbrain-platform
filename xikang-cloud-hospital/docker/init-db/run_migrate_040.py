#!/usr/bin/env python3
"""Apply migrate_040_communication_message_type_varchar.sql using xikang-cloud-hospital/.env DB profile."""
...
ENV_PATH = ROOT / ".env"
MIGRATION = ROOT / "docker" / "init-db" / "migrate_040_communication_message_type_varchar.sql"
...
def main() -> None:
    ...
    cur.execute(
        ...
        WHERE table_name = 'follow_up_communication_message'
          AND column_name = 'message_type'
        """
    )
    max_len = cur.fetchone()[0]
    print(f"Verification: message_type varchar max length => {max_len}")
    cur.close()
    conn.close()
