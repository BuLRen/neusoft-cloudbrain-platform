-- W4 门诊确诊工作流输出持久化：ai_medical_record_log.source_type 增加 w4_diagnose
-- 可重复执行（幂等）
...
BEGIN
    IF EXISTS (
        ...
        WHERE n.nspname = 'public'
          ...
          AND pg_get_constraintdef(c.oid) LIKE '%w4_diagnose%'
    ) THEN
        RAISE NOTICE 'chk_ai_mrlog_source 已包含 w4_diagnose，跳过迁移';
        RETURN;
    END IF;
