...
public class FollowUpStaffAuthInterceptor implements HandlerInterceptor {
    ...
    public FollowUpStaffAuthInterceptor(AuthUserMapper authUserMapper, ObjectMapper objectMapper) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!ALLOWED_ROLES.contains(role)) {
            writeError(response, 403, "无权访问随访系统接口");
            return false;
        }
        ...
        if (!isAdmin) {
            ...
            if (employeeId == null) {
                writeError(response, 403, "随访账号未绑定员工档案");
                return false;
            }
            ...
            if (departmentId == null) {
                writeError(response, 403, "随访账号未绑定科室");
                return false;
            }
        }
