...
import com.xikang.ctviewer.mapper.AuthUserMapper;
...
@Component
public class CtViewerAuthInterceptor implements HandlerInterceptor {
    ...
    public CtViewerAuthInterceptor(AuthUserMapper authUserMapper, ObjectMapper objectMapper) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!"medtech".equals(role) && !"admin".equals(role)) {
            writeError(response, 403, "无权访问 CT 影像接口");
            return false;
        }
        ...
        CtViewerAuthContext.set(new CtViewerAuthContext.Context(
            userId,
            role,
            employeeId,
            departmentId,
    ...
    public void afterCompletion(
        ...
        CtViewerAuthContext.clear();
    }

    private String extractToken(HttpServletRequest request) {
        if (request.getCookies() != null) {
            for (Cookie cookie : request.getCookies()) {
                if (ACCESS_COOKIE_NAME.equals(cookie.getName()) && cookie.getValue() != null && !cookie.getValue().isBlank()) {
