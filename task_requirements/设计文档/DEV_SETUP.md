...
| 人员 | 负责模块 | 职责 |
|------|----------|------|
| 人员A | 业务服务 | common, gateway, auth, registration, physician, medtech, pharmacy |
| 人员B | AI服务 | ai-gateway, ai-triage, ai-consult, ai-diagnosis, ai-pharmacy |
...
人员A和人员B可以同时开发各自负责的服务，互不依赖。

**阶段二：接口约定**

在开发前约定好 API 接口格式：
- AI 服务定义好被调用的接口（人员B负责）
- 业务服务按接口文档调用（人员A负责）

**阶段三：联调**

两边开发完成后进行联调：
- 人员A调用 ai-gateway-service 的接口
- 人员B确保 AI 服务返回正确格式

### 6.3 共享数据库

所有服务共用一个 PostgreSQL 数据库（xikang_hospital），
数据库表归属见 DEV_GUIDE_A.md。
