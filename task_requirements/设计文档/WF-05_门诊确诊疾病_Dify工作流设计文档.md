...
| 变量名 | 来源 |
|---|---|
| `disease_search_response` | 节点 5.body 或 HTTP 响应文本 |
...
def main(disease_search_response):
    response = safe_json_loads(disease_search_response)
...
| 变量名 | 来源 |
|---|---|
| `diagnosis_suggestions` | 节点 8A.diagnosisSuggestions |
| `clinical_summary_for_doctor` | 节点 8A.clinicalSummaryForDoctor |
| `differential_diagnosis` | 节点 8A.differentialDiagnosis |
| `warning_signs` | 节点 8A.warningSigns |
| `register_id` | 节点 2.register_id |
| `candidate_id_code_map_json` | 节点 6.candidate_id_code_map_json |

> 节点 8A 已启用结构化输出，此处直接接收数组/对象字段，不再解析 `text` JSON 字符串。
...
def main(
    ...
    suggestions = as_object_list(diagnosis_suggestions)
    ...
    return {
        ...
        "fallbackSuggestions": [],
        "clinicalSummaryForDoctor": str(clinical_summary_for_doctor or ""),
        "differentialDiagnosis": as_object_list(differential_diagnosis),
        "warningSigns": as_string_list(warning_signs),
        "searchAdvice": ""
    }
```
...
| 变量名 | 来源 |
|---|---|
| `fallback_suggestions` | 节点 8B.fallbackSuggestions |
| `clinical_summary_for_doctor` | 节点 8B.clinicalSummaryForDoctor |
| `search_advice` | 节点 8B.searchAdvice |
| `register_id` | 节点 2.register_id |

> 节点 8B 已启用结构化输出，此处直接接收数组/字符串字段，不再解析 `text` JSON 字符串。
...
def main(
    ...
    raw_suggestions = as_object_list(fallback_suggestions)
    ...
    return {
        ...
        "differentialDiagnosis": [],
        "warningSigns": [],
        "searchAdvice": str(search_advice or "")
    }
```
